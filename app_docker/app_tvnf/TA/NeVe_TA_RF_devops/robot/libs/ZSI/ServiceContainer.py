...
import urlparse, types, os, sys, StringIO, thread,re
...
### logs etc.
import time
import ThreadLogLogger
from ZSI.wstools import logging
from traceback import format_exc
logger = logging.getLogger("")
#logger.setLevel(10)
log = ThreadLogLogger.ThreadLogLogger()
###
...
def _Dispatch(ps, server, SendResponse, SendFault, post, action, nsdict={}, **kw):
    ...
    if isinstance(service, WSAResource):
        ...
        try:
            ...
            log.writeLog("[ZSI] Caught exception: %s (%s)" % (e, "_Dispatch"))
    ...
    try:
        ...
        log.writeLog("[ZSI] Caught exception: %s (%s)" % (str(e), "_Dispatch"))
        return SendFault(FaultFromException(e, 0, sys.exc_info()[2]), **kw)
    ...
    try:
        ...
        log.writeLog("[ZSI] Caught exception: %s (%s)" % (e, "_Dispatch"))
        # example  "[ZSI] Caught exception: I/O operation on closed file (_Dispatch)"
        return SendFault(FaultFromException(e, 0, sys.exc_info()[2]), **kw)
    ...
    def do_POST(self):
        ...
        try:
            ...
            if ct.startswith('multipart/'):
                ...
                #file.seek(0)
                self._cid = resolvers.MIMEResolver(ct, self.rfile)
                xml = cid.GetSOAPPart() #???
            ...
            log.writeLog("[ZSI] Caught exception: %s (%s)" % (e, "WSAResource/do_POST"))
            log.writeLog("[ZSI] Exception details: %s" % format_exc())
...
class SOAPRequestHandler(BaseSOAPRequestHandler):
    ...
    def do_POST(self):
        ...
        action -- SOAPAction(HTTP header) or wsa:Action(SOAP:Header)
        '''
        log.writeLog("HTTP POST start")
        
        soapAction = self.headers.getheader('SOAPAction')
        
        #log.writeLog(soapAction)
        
        post = self.path
        if not post:
            log.writeLog("HTTP POST not specified in request")
            raise PostNotSpecified, 'HTTP POST not specified in request'
        if soapAction:
            soapAction = soapAction.strip('\'"')
        post = post.strip('\'"')
        try:
            ct = self.headers['content-type']
            if ct.startswith('multipart/'):
                log.writeLog("Handling multipart content")
                str_file = StringIO.StringIO()                
                if self.headers.get('transfer-encoding') == 'chunked':
                    log.writeLog("transfer-encoding chunked")
                    chunk = 0                    
                    '''
                    Read chunks
                    '''                    
                    while True:
                        time.sleep(0.1)  # yield + delay                        
                        try:                            
                            hex_length = self.rfile.readline().strip()                           
                            '''
                            Skip empty lines
                            ''' 
                            if hex_length == '':                            
                            ...
                            try:
                                ...
                                log.writeLog("Invalid chunk size %s, error is %s " % (hex_length, e.message))
                                length = 0                                        
                            if length == 0:
                                break
                            log.writeLog("Reading chunk with length = %s" % str(length))                            
                            time.sleep(0.1) # yield + delay                            
                            str_file.write(self.rfile.read(length))                            
                        except IOError, (errno, strerror):
                            log.writeLog("I/O error(%s): %s" % (errno, strerror))
                        except ValueError:
                            log.writeLog("Invalid value error")
                        except:
                            log.writeLog("Unexpected error: %s" % sys.exc_info()[0])
                            raise                        
                        log.writeLog("Chunk done")
                                                
                    log.writeLog("Chunks handled")
                else:
                    log.writeLog("transfer-encoding not chunked")
                    length = int(self.headers['content-length'])
                    str_file.write(self.rfile.read(length))
                    
                str_file.seek(0)
                cid = resolvers.MIMEResolver(ct, str_file) 
                xml = cid.GetSOAPPart()
                
                log.writeLog(xml.getvalue())
                
                ps = ParsedSoap(xml, resolver=cid.Resolve)
                log.writeLog("Handling multipart content DONE")
            else:
                log.writeLog("Handling non-multipart content")
                length = int(self.headers['content-length'])
                xml = self.rfile.read(length)
                if xml.count('getvalue')>0:
                    log.writeLog(xml.getvalue())
                else:
                    log.writeLog(xml)
                
                ps = ParsedSoap(xml)
                log.writeLog("Handling non-multipart content DONE")

        except ParseException, e:
            log.writeLog("[ZSI] Caught exception: %s (%s)" % (e, "SOAPRequestHandler/do_POST"))
            log.writeLog("[ZSI] Exception details: %s" % format_exc())
            self.send_fault(FaultFromZSIException(e))
        except Exception, e:
            log.writeLog("[ZSI] Caught exception: %s (%s)" % (e, "SOAPRequestHandler/do_POST"))
            ...
            try:
                ...
                log.writeLog("_Dispatch OK")

            # Clean up after the call
            if _contexts.has_key(thread_id):
                del _contexts[thread_id]
        log.writeLog("HTTP POST end ")
