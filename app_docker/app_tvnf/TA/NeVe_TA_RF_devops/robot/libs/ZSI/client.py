...
#import ThreadLogLogger as logger 
...
class _Binding:
    ...
    def SendSOAPData(self, soapdata, url, soapaction, headers={}, **kw):
        ...
        log.writeLog('SOAP request content: %s'%(str(soapdata)))
        url = url or self.url
    ...
    def IsSOAP(self):
        if self.ps: return 1
        self.ReceiveRaw()
        mimetype = self.reply_headers.dict["content-type"]
        
        if mimetype.startswith("multipart/"):
            file = StringIO.StringIO()
            file.write(self.data)
            file.seek(0)
            cid = resolvers.MIMEResolver(mimetype, file) 
            
            self.data = cid.GetSOAPPart().getvalue()
            self.resolver = cid.Resolve
            
            return True
        else:
            self.resolver = None
            return self.reply_headers.type == 'text/xml'
    ...
    def ReceiveSOAP(self, readerclass=None, **kw):
        ...
        if not self.IsSOAP():
            raise TypeError(
                'Response is "%s", not "text/xml". Response data: %s' % (self.reply_headers.type, self.data))
        if len(self.data) == 0:
            raise TypeError('Received empty response')


        log.writeLog(self.data)
