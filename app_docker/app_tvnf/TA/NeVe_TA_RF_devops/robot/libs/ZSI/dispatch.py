    ...
    def do_POST(self):
        ...
        try:
            ...
            if ct.startswith('multipart/'):
                #position = self.rfile.tell()
                #self.rfile.seek(length)
                #self.rfile.write("\x04")
                #self.rfile.seek(position)
                log.writeLog("XXX in do_POST")
                file = open("/tmp/stream","w")
                file.write(self.rfile.read(length))
                file.close()
                file = open("/tmp/stream","r")
                cid = resolvers.MIMEResolver(ct, file) #self.rfile)
                xml = cid.GetSOAPPart()
