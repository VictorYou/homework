...
class Time:
    ...
    def __adjust(self):
        ...
        if hasattr(self, 'offset'):
            today = dt.date.today()
#            delta = self.tz.adjustment(self.offset) #linke:we do not need time offset
            d = dt.datetime.combine(today, self.time)
#            d = ( d + delta ) #linke:we do not need time offset
            self.time = d.time()
...
#        delta = self.tz.adjustment(self.offset) #linke:we do not need time offset
        try:
            d = self.datetime  #linke:we do not need time offset
#            d = ( self.datetime + delta ) #linke:we do not need time offset
#            self.datetime = d #linke:we do not need time offset
            self.date = d.date()
            self.time = d.time()
